...
def plot_daily_retention_cohorts(data):
    if not data:
    ...
    # Limit to 90 days
    pivot_df = pivot_df.loc[:, pivot_df.columns <= 90]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        textfont={"size": 10},
        hoverongaps=False,
        hovertemplate='Cohort: %{y}<br>Day: %{x}<br>Retention: %{z:.1f}%<extra></extra>',
        zmin=min_value,
        zmax=max_value
    ))
    
    fig.update_layout(
        title='Daily Cohort Retention Rates',
