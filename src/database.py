...
def init_session_tables(session_id):
    ...
    if isinstance(conn, SupabaseConnection):
        ...
        try:
            ...
            """
            conn.execute_sql(create_table_query)
            
            # Create indices
            create_index_query_1 = f"CREATE INDEX IF NOT EXISTS idx_revenue_date_{session_id} ON {table_name}(transaction_date);"
            create_index_query_2 = f"CREATE INDEX IF NOT EXISTS idx_revenue_user_{session_id} ON {table_name}(user_id);"
            conn.execute_sql(create_index_query_1)
            conn.execute_sql(create_index_query_2)
