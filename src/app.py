                        ...
                        with col1:
                            if st.button("Generate Charts", use_container_width=True):
                                try:
                                    ...
                                    total_rows = len(df)
                                    chunk_size = max(1, total_rows // 50) 
                                    processed_rows = 0
                                    ...
                                    progress_bar.empty()
                                    status_text.empty()
                                    st.success(f"Successfully stored {total_rows:,} records!")
                                    
                                    # Set flags to automatically apply filters on initial data load
                                    st.session_state.filters_applied = True
                                    
                                    # Replace the multiple data fetching calls with single call
