...
def main():
    ...
    tab1, tab2, tab3, retention_tab, quick_ratio_tab, tab7 = st.tabs([
        "Data Upload", 
        "Revenue Growth",
        "User Growth",
        "Retention over Period",
        "Quick Ratio",
        "Cohorts"
    ])
    ...
    with tab1:
        ...
        with main_col1:
            ...
            st.markdown("""
                ...
                - See example file below to know the expected format or test the app with it
            """)
    ...
    with tab2:
        ...
        # Only fetch and display data when either:
        # 1. Data was just generated (should_apply_filters is True)
        # 2. Data exists and user clicked Apply filters
    ...
    with retention_tab:
        st.subheader("Retention over Period")
        
        # Create a single row for all filters
        filter_col1, filter_col2, filter_col3, filter_col4 = st.columns([1, 0.8, 0.8, 0.8])
        
        with filter_col1:
            st.markdown("Select Period", help="Choose the time period for retention analysis")
            retention_frequency = st.selectbox(
                "",
                options=["Monthly", "Weekly", "Daily"],
                key="combined_retention_frequency",
    ...
    with quick_ratio_tab:
        ...
        with filter_col1:
            st.markdown("Select Period", help="Choose the time period for quick ratio analysis")
            quick_ratio_frequency = st.selectbox(
                "",
                options=["Monthly", "Weekly", "Daily"],
                key="combined_quick_ratio_frequency",
                label_visibility="collapsed"
            )
        ...
        with filter_col4:
            ...
            apply_filters = st.button(
                "Apply filters",
                key="quick_ratio_apply",
                use_container_width=True
            )
        
        # Add benchmark explanations right after filters
    ...
    with tab7:
        ...
        # Create a single row for all filters
        filter_col1, filter_col2, filter_col3, filter_col4, filter_col5 = st.columns([1, 0.8, 0.8, 0.8, 0.8])
        
        with filter_col1:
            st.markdown("Select Period", help="Choose the time period for cohort analysis")
            cohort_frequency = st.selectbox(
                "",  # Empty label since we're using the markdown above
                options=["Monthly", "Weekly", "Daily"],
        ...
        if (st.session_state.should_apply_filters and st.session_state.data_generated) or \
           (st.session_state.data_generated and apply_filters):
            ...
            st.session_state.last_cohort_frequency = cohort_frequency
            
            if cohort_frequency == "Monthly":
                st.session_state.monthly_cohort_data = get_monthly_cohorts(st.session_state.session_id, start_date, end_date)
            elif cohort_frequency == "Weekly":
                st.session_state.weekly_cohort_data = get_weekly_cohorts(st.session_state.session_id, start_date, end_date)
            else:  # Daily
                st.session_state.daily_cohort_data = get_daily_cohorts(st.session_state.session_id, start_date, end_date)

        if st.session_state.data_generated and hasattr(st.session_state, 'last_cohort_frequency'):
            # Show appropriate note based on last applied frequency
            if st.session_state.last_cohort_frequency == "Monthly":
                st.info("Note: Limited to 24 months since first purchase")
            elif st.session_state.last_cohort_frequency == "Weekly":
                st.info("Notes:\n"
            ...
            with col1:
                ...
                if st.session_state.last_cohort_frequency == "Monthly" and 'monthly_cohort_data' in st.session_state:
                    plot_cohorts(st.session_state.monthly_cohort_data)
                elif st.session_state.last_cohort_frequency == "Weekly" and 'weekly_cohort_data' in st.session_state:
                    plot_weekly_retention_cohorts(st.session_state.weekly_cohort_data)
                elif st.session_state.last_cohort_frequency == "Daily" and 'daily_cohort_data' in st.session_state:
                    plot_daily_retention_cohorts(st.session_state.daily_cohort_data)
            
            with col2:
                st.markdown("##### LTV")
                if st.session_state.last_cohort_frequency == "Monthly" and 'monthly_cohort_data' in st.session_state:
                    plot_ltv_cohorts(st.session_state.monthly_cohort_data)
                elif st.session_state.last_cohort_frequency == "Weekly" and 'weekly_cohort_data' in st.session_state:
                    plot_weekly_ltv_cohorts(st.session_state.weekly_cohort_data)
                elif st.session_state.last_cohort_frequency == "Daily" and 'daily_cohort_data' in st.session_state:
                    plot_daily_ltv_cohorts(st.session_state.daily_cohort_data)
